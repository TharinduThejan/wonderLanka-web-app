...
export default function DetailPage() {
  ...
  return (
    <div className="bg-[#121212] min-h-screen text-white font-sans antialiased selection:bg-[#4ADE80] selection:text-black overflow-x-hidden">
      ...
      {/* 2. MAIN CENTERED CONTENT (Description, Info, Slider) */}
      <div className="max-w-5xl mx-auto px-4 sm:px-8 py-12 flex flex-col gap-12">
        ...
        <div>
          ...
          <div className="relative w-full h-[400px] md:h-[600px] bg-black rounded-3xl overflow-hidden group border border-white/10 shadow-2xl">
            <div 
            ...
            <button 
              onClick={prevSlide}
              className="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-black/50 backdrop-blur-sm rounded-full flex items-center justify-center border border-white/10 text-white opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/80"
            >
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 19l-7-7 7-7"/></svg>
            </button>
            <button 
              onClick={nextSlide}
              className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-black/50 backdrop-blur-sm rounded-full flex items-center justify-center border border-white/10 text-white opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/80"
            >
            ...
            <div className="absolute bottom-6 left-0 w-full flex justify-center gap-2">
              {galleryImages.map((_, idx) => (
                <button 
                  ...
                  className={`h-1.5 rounded-full transition-all duration-300 ${currentSlide === idx ? 'w-8 bg-[#4ADE80]' : 'w-2 bg-white/50 hover:bg-white'}`}
                />
              ))}
            </div>
          </div>
